...
<Apps topdata={topdata} appname="boosting">
    <div class="boosting">
        ...
        <div class="top">
            <div class="navigation">
                {#each pages as page}
                <button class:green={page === 'My Contracts'} class:red={page === "Buy Contracts"} class:active={page === activepage} on:click={
                    (e) => setActivePage(e, page)
                }>{page}</button>
                {/each}
                <button class="blue" on:click={joinQueue} class:disabled={iswaiting}>
                    {#if iswaiting}
                       <i class="fas fa-regular fa-circle-notch fa-spin"></i>
                    {:else}
